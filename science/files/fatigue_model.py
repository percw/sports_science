...
PARAM_GROUPS = {
    "onset_time_constants": ["tau_a_on", "tau_n_on", "tau_c_on", "tau_w_on"],
    "recovery_time_constants": ["tau_a_off", "tau_n_off", "tau_c_off", "tau_w_off"],
    "state_dependence": ["alpha_a", "alpha_n", "alpha_c"],
    "cross_talk": ["kappa_na", "kappa_an", "kappa_mc"],
    "load_scaling": ["load_scale_a", "load_scale_n", "load_scale_c", "load_scale_w"],
...
FIT_PROFILES = {
    "dynamics_core": ["recovery_time_constants", "state_dependence"],
    "coupling_core": ["cross_talk"],
    "observation_core": ["observation_baselines", "observation_loadings"],
    "measurement_core": ["observation_noise"],
    "load_core": ["load_scaling"],
...
class FatigueModelParams:
    ...
    @classmethod
    def group_names(cls):
        return list(PARAM_GROUPS.keys())

    @classmethod
    def fit_profile_names(cls):
        return list(FIT_PROFILES.keys())

    @classmethod
    def params_for_groups(cls, groups):
        names = []
        seen = set()
        for group in groups:
            for name in PARAM_GROUPS.get(group, []):
                if name not in seen:
                    names.append(name)
                    seen.add(name)
        return names
    ...
    @classmethod
    def params_for_fit_profiles(cls, profiles):
        names = []
        seen = set()
        for profile in profiles:
            for name in cls.params_for_groups(FIT_PROFILES.get(profile, [])):
                if name not in seen:
                    names.append(name)
                    seen.add(name)
        return names
    ...
    @classmethod
    def group_for_param(cls, param_name):
    ...
    @classmethod
    def grouped_params(cls, names=None):
        names = names or cls.param_names()
        grouped = {}
        for name in names:
            group_name = cls.group_for_param(name)
    ...
    def bounds(self):
        ...
        for name in self.__dataclass_fields__:
            ...
            if name.startswith("tau_"):
                ...
                lower = max(1.0, val * 0.3)
                upper = max(lower + 1.0, val * 3.0, 3.0)
                b.append((lower, upper))
...
def resolve_fit_subset(fit_subset=None, fit_groups=None, fit_profiles=None):
    names = []
    seen = set()

    for source in [
        fit_subset or [],
        FatigueModelParams.params_for_groups(fit_groups or []),
        FatigueModelParams.params_for_fit_profiles(fit_profiles or []),
    ]:
        for name in source:
            if name not in seen:
                names.append(name)
...
def fit_model(inputs, initial_params=None, max_iter=500, fit_subset=None, fit_groups=None, fit_profiles=None):
    ...
    resolved_subset = resolve_fit_subset(fit_subset=fit_subset, fit_groups=fit_groups, fit_profiles=fit_profiles)
    fit_indices = [all_names.index(n) for n in resolved_subset] if resolved_subset else list(range(len(all_names)))
    ...
    info = {
        ...
        "fit_subset": [all_names[i] for i in fit_indices],
        "fit_groups": sorted({FatigueModelParams.group_for_param(name) for name in [all_names[i] for i in fit_indices]}),
    }
    if n_obs > 0:
